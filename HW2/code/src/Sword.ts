...
export class Sword extends Weapon {
  ...
  if (damageModifier < damageLimit) {
    if ((damageModifier + this.getModifierRate()) > damageLimit) {
        ...
        return `Your ${this.getName()} is polished to the max value now. Value of damageModifier is ${this.getDamageModifier().toFixed(2)}.`;
      } else {
        this.setDamageModifier(damageModifier + this.getModifierRate());
        return `Your ${this.getName()} was polished. Value of damageModifier is ${this.getDamageModifier().toFixed(2)}.`;
      }
    } else {
      return `Your ${this.getName()} is polished enough. Value of damageModifier is ${this.getDamageModifier().toFixed(2)}.`;
    }
  }
}
